...
class MarkerDetectionNode:
    def __init__(self):
        ...
        self.camera_publisher = rospy.Publisher('/robot_cam/joint1_position_controller/command' , Float64 , queue_size = 1)
    ...
    def ImageCallback(self, msg):
        ...
        if self.SearchingForMarkers:
            ...
            self.CurrentAngle += 5
            rad = math.radians(self.CurrentAngle)  # Convert degrees to radians
            self.camera_publisher.publish(rad)
            time.sleep(0.1)
                ...
                if self.CameraInfo is not None:
                    ...
                    if self.TargetID == self.CurrentMarkerID:
                        ...
                        if abs(OffsetX) < 5:
                            ...
                            self.CurrentAngle += 1.0  # Adjust orientation based on horizontal offset
                            rad = math.radians(self.CurrentAngle)  # Convert degrees to radians
                            self.camera_publisher.publish(rad)
                            time.sleep(0.1)
        ...
        print("the angle published :", self.CurrentAngle) 
        time.sleep(0.1)
